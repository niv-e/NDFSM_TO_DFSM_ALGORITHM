...
*/
	...
	public DFSM toDFSM(NDFSM ndfsm) {
		/*
		 * The algurithom
		 * for each state q of k_m compute eps(q)
		 * s' = eps(s)
		 * compute lambda
		 * 
		 * */
		State state = ndfsm.initialState;
		ndfsm.getEpsof(ndfsm , state);
		
		return null;
	}

	private Set<State> getEpsof(NDFSM ndfsm ,State state) {
		// TODO Auto-generated method stub
		Set <State> eps_trans = ndfsm.transitions.at(state, Alphabet.EPSILON );
		if(eps_trans.isEmpty()) {
			Set<State> s = new HashSet<State>();
			s.add(state);
			return s;
		}
		for(State stateS : eps_trans )
			eps_trans.addAll(getEpsof(ndfsm,stateS));
		return eps_trans;
